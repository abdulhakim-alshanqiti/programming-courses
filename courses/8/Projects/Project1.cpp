...
void PrintUserPerms(stUser User) {
	//User.Perms.ViewClientList = (UserPerms & ViewClientListPerm);
	//User.Perms.AddClient = (UserPerms & AddClientPerm);
	//User.Perms.DeleteClient = (UserPerms & DeleteClientPerm);
	//User.Perms.UpdateClient = (UserPerms & UpdateClientPerm);
	//User.Perms.FindClient = (UserPerms & FindClientPerm);
	//User.Perms.ViewTransactions = (UserPerms & ViewTransactionsPerm);
	//User.Perms.ManageUsers = (UserPerms & ManageUsersPerm);


	Printl("Can User View Client List ?" + BoolToTrueAndFalse(User.Perms.ViewClientList));
	Printl("Can User Add Client ?" + BoolToTrueAndFalse(User.Perms.AddClient));
	Printl("Can User Delete Client ?" + BoolToTrueAndFalse(User.Perms.DeleteClient));
	Printl("Can User Update Client ?" + BoolToTrueAndFalse(User.Perms.UpdateClient));
	Printl("Can User Find Client ?" + BoolToTrueAndFalse(User.Perms.FindClient));
	Printl("Can User View Transactions ?" + BoolToTrueAndFalse(User.Perms.ViewTransactions));
	Printl("Can User Manage Users ?" + BoolToTrueAndFalse(User.Perms.ManageUsers));
}
...
//	short UserPerms = 0;
//	if (true) {
//		UserPerms += ViewClientListPerm + UpdateClientPerm + AddClientPerm;
//	}
//
//
//	PrintUserPerms(User, UserPerms);
//}
...
bool FindUserByUsernameAndPassword(stUser& TempUserRecord, vector<stUser>& vUsersRecords, string UsernameToSearchFor, string Password) {



	for (stUser& UserRecord : vUsersRecords)
	{
		if (UserRecord.Username == UsernameToSearchFor && UserRecord.Password == Password)
		{
			TempUserRecord = UserRecord;
			return true;
		}
	}
